...
from tic_tac_toe_game.helpers import positions_mapper
...
def play_turn(board, sign, selected_pos):
    if position_available(board, selected_pos):
        ...
        current_selected_position = input(f"Select a valid available position number between 1-9: ")
...
def check_selected_position_is_valid(pos, counter):
    if not pos.isdigit() or not is_position_valid(pos):
        current_selected_position = input(
            f"{setup.player_names[counter % 2]}, select a position number between 1-9: ")
...
def play():
    ...
    while not has_won() and has_moves(turns_counter):
        current_selected_position = input(f'{setup.player_names[turns_counter % 2]}, '
                                          f'select a position number between 1-9: ')
    ...
    turns_counter -= 1
    print(f"Congrats, {setup.player_names[turns_counter % 2]} you won!")
